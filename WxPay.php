...
class WxPay extends Common {
     ...
     * 	作用：生成签名
     */
    public function createSign($Obj)
    {
        foreach ($Obj as $k => $v)
        {
            $Parameters[$k] = $v;
 ...
 * 请求型接口的基类
 */
class WxpayClient extends WxPay
{
...
class OrderQuery extends WxpayClient
{
	...
	function createXml()
	{
		try
		{
			...
			{
				throw new RuntimeException("订单查询接口中，out_trade_no、transaction_id至少填一个！"."<br>");
			}
		   	$this->parameters["appid"] = Conf::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = Conf::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null && $this->parameters["transaction_id"] == null) {
				throw new RuntimeException("退款申请接口中，out_trade_no、transaction_id至少填一个！"."<br>");
			}elseif($this->parameters["out_refund_no"] == null){
				throw new RuntimeException("退款申请接口中，缺少必填参数out_refund_no！"."<br>");
			}elseif($this->parameters["total_fee"] == null){
				throw new RuntimeException("退款申请接口中，缺少必填参数total_fee！"."<br>");
			}elseif($this->parameters["refund_fee"] == null){
				throw new RuntimeException("退款申请接口中，缺少必填参数refund_fee！"."<br>");
			}elseif($this->parameters["op_user_id"] == null){
				throw new RuntimeException("退款申请接口中，缺少必填参数op_user_id！"."<br>");
			}
		   	$this->parameters["appid"] = Conf::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = Conf::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
...
class RefundQuery extends WxpayClient
{
	...
	function createXml()
	{
		try
		{
			...
			{
				throw new RuntimeException("退款查询接口中，out_refund_no、out_trade_no、transaction_id、refund_id四个参数必填一个！"."<br>");
			}
		   	$this->parameters["appid"] = Conf::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = Conf::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
...
class DownloadBill extends WxpayClient
{
	...
	function createXml()
	{
		try
		{
			...
			{
				throw new RuntimeException("对账单接口中，缺少必填参数bill_date！"."<br>");
			}
		   	$this->parameters["appid"] = Conf::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = Conf::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
...
class ShortUrl extends WxpayClient
{
	...
	function createXml()
	{
		try
		{
			...
			{
				throw new RuntimeException("短链接转换接口中，缺少必填参数long_url！"."<br>");
			}
		   	$this->parameters["appid"] = Conf::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = Conf::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
 ...
 * 响应型接口基类
 */
class WxpayServer extends WxPay
{
...
class NativeCall extends WxpayServer
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = Conf::APPID;//公众账号ID
		   	$this->returnParameters["mch_id"] = Conf::MCHID;//商户号
		    $this->returnParameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->returnParameters["sign"] = $this->createSign($this->returnParameters);//签名
		}
		return $this->arrayToXml($this->returnParameters);
	}
 ...
 * 静态链接二维码
 */
class NativeLink  extends WxPay
{
	...
	function createLink()
	{
		try
		{
			...
			{
				throw new RuntimeException("缺少Native支付二维码链接必填参数product_id！"."<br>");
			}
		   	$this->parameters["appid"] = Conf::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = Conf::MCHID;//商户号
		   	$time_stamp = time();
